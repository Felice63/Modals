...
// 2] I used the 'for/of' method instead of the forEach() method to iterate over each nodelist because it gets values instead of properties.
// 'for/of' works on any iterable object.
// The forEach() method is typically used with arrays because it works on the indvidual indices of the array. This is the same with the for/in(method) which is also used with indices and numeric keys. A NodeList is not an array, but it has indexes for each node and the length property can be used
